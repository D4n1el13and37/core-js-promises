...
function getPromiseResult(source) {
  return new Promise((res) => {
    source.then(() => res('success'));
    source.catch(() => res('fail'));
  });
}
...
function getAllResult(promises) {
  return Promise.allSettled(promises).then((promiseData) => {
    return promiseData.map((resolve) =>
      resolve.status === 'rejected' ? null : resolve.value
    );
  });
}
